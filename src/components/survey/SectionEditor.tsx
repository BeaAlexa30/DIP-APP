            ...
            {otherSections.length > 0 && (
              <div className="border-t border-gray-100">
                ...
                {otherSections.map(sec => {
                  const isSelected =
                    currentType === 'jump' && action?.targetSectionId === sec.id
                  return (
                    <button
                      key={sec.id}
...
export default function SectionEditor({
  ...
  /** Update a single option's action for a question */
  function handleOptionActionChange(
    q: any,
    optId: string,
    newAction: OptionAction,
  ) {
  ...
  /** True when the question type supports per-option jump logic */
                      ...
                      >
                        ...
                        {isQuestionExpanded && (
                          <div className="mt-2 p-3 bg-violet-50 border border-violet-200 rounded space-y-3">
                            ...
                            {/* Options + Jump Logic for selection-based questions */}
                            {['multiple_choice', 'checkboxes', 'dropdown'].includes(q.type) && q.options && (
                              <div className="pt-3 border-t border-violet-200 space-y-3">
                                ...
                                {/* Column headers when jump logic is active */}
                                {hasJumpLogic && (
                                  ...
                                  {q.options.map((opt: any, optIdx: number) => {
                                    ...
                                    return (
                                      <div
                                        ...
                                        onDragOver={e => { e.preventDefault(); setOptDragOverIndex(optIdx) }}
                                        onDrop={() => {
                                      ...
                                      >
                                        ...
                                        {!isOther && (
                                          <div
                                            className="w-4 h-4 bg-gray-200 rounded shrink-0 cursor-grab active:cursor-grabbing flex items-center justify-center text-gray-400 text-[10px]"
                                            onMouseDown={e => {
                                              const parent = e.currentTarget.closest('[draggable]') as HTMLElement
                                              if (parent) parent.draggable = true
                                            }}
