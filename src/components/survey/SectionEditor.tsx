...
export default function SectionEditor({
  ...
  function handleSectionDragOver(e: React.DragEvent) {
    // Allow section drag events to bubble up to the parent wrapper
    if (!isDragging) return
    // Only intercept if this is a question drag
  ...
  function handleSectionDrop(e: React.DragEvent) {
    // Only intercept question drags, let section drags bubble to parent
    if (!isDragging) return
    e.preventDefault()
    e.stopPropagation()
        ...
        <>
          <div className="border-t border-blue-200 px-4 py-4 space-y-4">
              ...
              <div className="pt-3 border-t border-blue-100">
                ...
                <div
                  className={`space-y-2 rounded-lg border p-3 transition-colors ${
                    sectionIsActiveDrop ? 'border-violet-300 bg-violet-50/40' : 'bg-white border-blue-100'
                  }`}
              ...
              {(section.sectionButtons || []).length > 0 && (
                <div className="space-y-2 mb-3">
                  {(section.sectionButtons || []).map(btn => {
                    ...
                    return (
                      <div key={btn.id} className="flex items-center justify-between gap-2 bg-white border border-blue-200 rounded p-2">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900">{btn.label}</p>
                          <p className="text-xs text-gray-500">→ {targetSection?.name || 'Unknown Section'}</p>
                        </div>
                        <button
                ...
                <div className="bg-blue-50 border border-blue-200 rounded p-3 space-y-2">
                  ...
                  >
                    ...
                    {allSections.filter(s => s.id !== section.id).map(s => (
                      <option key={s.id} value={s.id}>{s.name || `Section ${s.order}`}</option>
                    ))}
                  </select>
                  <div className="flex gap-2">
                    <button
                    ...
                    <button
                      onClick={() => { setShowAddButton(false); setButtonLabel(''); setButtonTarget('') }}
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-blue-100">
              ...
              />
              <label htmlFor={`sec-collapse-${section.id}`} className="text-sm text-gray-700 cursor-pointer">
